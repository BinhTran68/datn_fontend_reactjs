...
import { Upload, DatePicker, Input, Select, Radio, Button, message, Modal, Form, Avatar, Drawer } from 'antd';
import { UserOutlined, CheckCircleOutlined, EditOutlined, PlusOutlined } from '@ant-design/icons';
...
const CustomerProfile = () => {
  ...
  const [address, setAddress] = useState({ // State cho địa chỉ MẶC ĐỊNH ở chế độ CHỈNH SỬA
    provinceId: null,
    districtId: null,
    wardId: null,
  ...
  const [newAddress, setNewAddress] = useState({ // State cho form THÊM/SỬA trong Drawer
    provinceId: null,
    districtId: null,
    wardId: null,
    specificAddress: null
  ...
  // --- Trạng thái mới ---
  const [isAddressListModalVisible, setIsAddressListModalVisible] = useState(false); // Cho Modal Danh sách Địa chỉ

  useEffect(() => {
    const id = localStorage.getItem('customerId') || 3; // ID ví dụ
    setCustomerId(id);
    if (id) {
    ...
    try {
      ...
      const processedAddresses = await Promise.all(
       ...
       if (defaultAddress) {
           ...
           setAddress({ provinceId: null, districtId: null, wardId: null, specificAddress: null });
      ...
      if (customerData.avatar) {
        ...
        setFileList([{
          ...
          name: 'avatar', status: 'done', url: customerData.avatar,
        }]);
      } else {
         setAvatar(null);
        ...
        if (axios.isAxiosError(err) && err.response) {
          ...
          setError(`Lỗi ${err.response.status}: Không thể tải thông tin khách hàng. Chi tiết: ${err.response.data.message || err.message}`);
  ...
  const handleAddNewAddressClick = () => {
    ...
    setNewAddress({ // Đặt lại trạng thái form
      provinceId: null,
      districtId: null,
      wardId: null,
      specificAddress: ""
  ...
  const handleEditAddressClick = (addressToEdit) => {
    ...
    setNewAddress({ // Điền trước trạng thái form
      provinceId: addressToEdit.provinceId ? parseInt(addressToEdit.provinceId) : null,
      districtId: addressToEdit.districtId ? parseInt(addressToEdit.districtId) : null,
  ...
  const handleAddAddress = async () => {
    ...
    // Xác thực cơ bản
     if (!newAddress.provinceId || !newAddress.districtId || !newAddress.wardId || !newAddress.specificAddress?.trim()) {
    ...
    try {
      ...
      message.error(`Thêm địa chỉ thất bại! ${error.response?.data?.message || ''}`);
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleUpdateAddress = async () => {
    ...
    try {
      ...
      message.error(`Cập nhật địa chỉ thất bại! ${error.response?.data?.message || ''}`);
    } finally {
        setLoading(false);
    }
  };
  ...
  const handleSetDefaultAddress = (addressId) => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        message.error(`Đặt làm mặc định thất bại! ${error.response?.data?.message || ''}`);
  ...
  const handleAddressChange = (province, district, ward, specific) => {
     if (editMode) { // Nếu đang ở chế độ chỉnh sửa hồ sơ chính, cập nhật state 'address'
         setAddress(prev => ({
             ...prev,
     ...
     } else if (isDrawerOpen) { // Nếu Drawer Thêm/Sửa đang mở, cập nhật state 'newAddress'
         setNewAddress(prev => ({
             ...prev,
  ...
  // Những hàm này phần lớn nên giữ nguyên, nhưng đảm bảo `saveCustomerData` xử lý đúng state `address` cho việc cập nhật/thêm địa chỉ mặc định.
    ...
    const getStatusBadge = () => {
      ...
      return formData.status === 'Kích hoạt' ? styles.statusBadgeActive : styles.statusBadgeInactive;
    };
      ...
      const handleRemove = async (file) => {
         ...
         try {
             ...
             return true; // Xác nhận xóa
              ...
              console.error('Lỗi xóa ảnh từ Cloudinary:', error.response?.data || error.message);
    ...
    const saveCustomerData = async () => {
         ...
         try {
             ...
             let finalAvatarPublicId = avatarPublicId; // Bắt đầu với public ID hiện có
              ...
              if (fileList.length > 0 && fileList[0].status === 'done' && fileList[0].response) {
                 ...
                 if (avatarPublicId && finalAvatarPublicId !== avatarPublicId) {
                     console.log(`New avatar uploaded (${finalAvatarPublicId}), previous one (${avatarPublicId}) might need deletion if not handled by onRemove.`);
                     // Cân nhắc thêm logic xóa ở đây nếu `handleRemove` không đảm bảo
                      try {
                        await axios.post('http://localhost:8080/cloudinary/delete', { public_id: avatarPublicId });
                 ...
                 console.log(`Avatar removed (${avatarPublicId}), should have been deleted by handleRemove. Setting avatar to null.`);
                 finalAvatarUrl = null;
             ...
             const customerUpdateData = {
                 ...
                 dateBirth: editFormData.birthDate ? editFormData.birthDate.toISOString() : null, // Sử dụng định dạng ISO cho backend
             ...
             await axios.put(`http://localhost:8080/api/admin/customers/update/${customerId}`, customerUpdateData);
             console.log("Customer basic info updated.");

              // 4. Xử lý Cập nhật/Thêm Địa chỉ Mặc định từ Form Chỉnh sửa
              const hasAddressInput = address.provinceId && address.districtId && address.wardId && address.specificAddress?.trim();
              ...
              if (defaultAddressId) {
                  ...
                  if (hasAddressInput) {
                      ...
                      await axios.put(`http://localhost:8080/api/admin/customers/update-address/${defaultAddressId}`, addressPayload);
                      console.log("Existing default address updated.");
                  } else {
                       console.log("Default address exists, but input fields are empty/incomplete in edit form. No address update performed.");
                  ...
                  await axios.post(`http://localhost:8080/api/admin/customers/add-address/${customerId}`, addressPayload);
                  console.log("New default address added.");
                   ...
                   // Không có địa chỉ mặc định tồn tại, và không có thông tin địa chỉ mới nào được cung cấp trong form chỉnh sửa. Không làm gì liên quan đến địa chỉ.
                    console.log("No existing default address and no address provided in edit form.");
               }
             ...
             message.error(`Cập nhật thông tin thất bại! ${error.response?.data?.message || ''}`);
         } finally {
  ...
  const renderContent = () => {
    switch (activeMenu) {
      ...
      case 'financial':
        return (
          <>
            <div className={styles.sectionHeader}><h2>Thông tin tài chính</h2></div>
            <div className={styles.infoCard}><p>Nội dung thông tin tài chính...</p></div>
          </>
        );
      case 'history':
        return (
          <>
            <div className={styles.sectionHeader}><h2>Lịch sử mua hàng</h2></div>
            <div className={styles.infoCard}><p>Nội dung lịch sử mua hàng...</p></div>
          </>
        );
       ...
       case 'feedback':
        return (
            <>
                <div className={styles.sectionHeader}><h2>Đánh giá & Phản hồi</h2></div>
                <div className={styles.infoCard}><p>Nội dung đánh giá & phản hồi...</p></div>
            </>
        );
        case 'marketing':
        return (
            <>
                <div className={styles.sectionHeader}><h2>Thông tin tiếp thị</h2></div>
                <div className={styles.infoCard}><p>Nội dung thông tin tiếp thị...</p></div>
            </>
        );
        case 'preferences':
        return (
            <>
                <div className={styles.sectionHeader}><h2>Sở thích & Nhu cầu</h2></div>
                <div className={styles.infoCard}><p>Nội dung sở thích & nhu cầu...</p></div>
            </>
        );
        case 'category':
        return (
            <>
                <div className={styles.sectionHeader}><h2>Phân loại khách hàng</h2></div>
                <div className={styles.infoCard}><p>Nội dung phân loại khách hàng...</p></div>
            </>
        );
      default:
        return (
          <>
            <div className={styles.sectionHeader}><h2>Thông tin cá nhân</h2></div>
            <div className={styles.infoCard}><p>Vui lòng chọn một mục để xem chi tiết.</p></div>
          </>
        );
    }
  };
  ...
  const renderViewForm = () => {
    ...
    const defaultAddressDisplay = formData.address || 'Chưa có địa chỉ mặc định';
      ...
      <div className={styles.formGrid}>
        ...
        <div className={`${styles.formGroup} ${styles.addressViewContainer}`}>
            ...
            <div className={styles.addressDisplay}>
                <span className={styles.formValue} style={{ marginRight: '10px', flexGrow: 1 }}>
    ...
    return (
      <div className={styles.editFormContainer}>
          ...
          <div className={styles.avatarEditSection}>
            ...
            >
                {fileList.length > 0 && fileList[0].url ? (
                     <img src={fileList[0].url} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
           ...
           <div className={styles.formGroup} style={{ gridColumn: '1 / span 2' }}>
             ...
             {isAddressLoaded ? (
                 <AddressSelectorAntd
                     ...
                     provinceId={address.provinceId ? parseInt(address.provinceId) : undefined}
                     districtId={address.districtId ? parseInt(address.districtId) : undefined}
             ...
             <small>Chỉnh sửa tại đây sẽ cập nhật địa chỉ mặc định hoặc thêm mới nếu chưa có.</small>
           </div>
        </div>
        ...
        <div className={styles.formActions}>
          ...
          <Button onClick={cancelEdit} style={{ marginLeft: '10px' }} disabled={loading}>
            Hủy
          </Button>
        </div>
      </div>
  ...
  return (
    <div className={styles.container}>
         ...
         <div className={styles.sidebar}>
           <div className={styles.profileCard}>
                 ...
                 {/* Hiển thị Avatar */}
                  <Avatar
                   ...
                   >
                      ...
                      {!avatar && formData.fullName ? formData.fullName.charAt(0).toUpperCase() : null}
                   </Avatar>
                 <div className={`${styles.statusBadge} ${getStatusBadge()}`}>{formData.status || 'N/A'}</div>
              </div>
             ...
             <div className={styles.menuItems}>
               {['personal', 'financial', 'history', 'feedback', 'marketing', 'preferences', 'category'].map((item) => {
      ...
      <Modal
        ...
        footer={[ // Footer tùy chỉnh
          <Button key="add" type="primary" icon={<PlusOutlined />} onClick={handleAddNewAddressClick}>
            Thêm địa chỉ mới
          </Button>,
          <Button key="close" onClick={() => setIsAddressListModalVisible(false)}>
            Đóng
          </Button>,
        ]}
            ...
            addresses.map((addressObj) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: '5px',
                  border: `1px solid ${addressObj.isDefault ? '#1890ff' : '#d9d9d9'}`, // Đánh dấu địa chỉ mặc định
                  marginBottom: '10px',
              ...
              >
                <span style={{ flex: 1, marginRight: '10px' }}>
                    {addressObj.address}
                    {addressObj.isDefault && <CheckCircleOutlined style={{ color: '#52c41a', marginLeft: '8px' }} title="Địa chỉ mặc định"/>}
                </span>
                <div>
                    {/* Nút Sửa */}
                    <Button
      ...
      <Drawer
        ...
        footer={ // Thêm footer cho các nút
            <div style={{ textAlign: 'right' }}>
                <Button onClick={() => setIsDrawerOpen(false)} style={{ marginRight: 8 }}>
                    Hủy
                </Button>
                <Button onClick={handleSaveAddressFromDrawer} type="primary" loading={loading}>
      ...
      >
        ...
        <Form layout="vertical" key={editingAddressId || 'new'}> {/* Thêm key để buộc mount lại */}
          <Form.Item
           ...
           <AddressSelectorAntd
             ...
             provinceId={newAddress.provinceId ? parseInt(newAddress.provinceId) : undefined}
             districtId={newAddress.districtId ? parseInt(newAddress.districtId) : undefined}
             wardId={newAddress.wardId ? parseInt(newAddress.wardId) : undefined}
